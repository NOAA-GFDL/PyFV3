...
from ndsl.dsl.gt4py import BACKWARD, FORWARD, PARALLEL, computation, cos
from ndsl.dsl.gt4py import function as gtfunction
from ndsl.dsl.gt4py import interval, max, sin
from ndsl.dsl.typing import Float, FloatField, FloatFieldIJ
from ndsl.grid import GridData
from pyfv3.stencils.fv_phys._config import GrayRadiationConfig


T0E = 8.0  # Dargan value= 6.
T0P = 1.5
FL = 0.1
SBC = 5.6734e-8
CP = 1004.64
...
@gtfunction
def compute_tau(phalf: FloatField, ps: FloatFieldIJ, tau0: FloatFieldIJ):
    from __externals__ import strat_off

    if not strat_off:
        # Dargan version:
        sig = phalf[0, 0, 0] / ps[0, 0]
        tau = tau0[0, 0] * (sig * FL + (1.0 - FL) * sig**4)
    else:
...
def gray_radiation(
    ...
    from __externals__ import diurnal_cycle, dt_atmos, shift_n, sw_rad
    ...
    with computation(FORWARD), interval(0, 1):
        ...
        if diurnal_cycle:
            ...
            sw_surf[0, 0] = (
                ...
                * (1.0 - clouds[0, 0])
                * cos(lat[0, 0])
                * max(0.0, cos(solar_ang[0, 0]))
            )
            sw_surf[0, 0] = sw_surf[0, 0] * (1.0 - ALBD)
        else:
            sw_surf = (
                sw_rad
                * (1.0 - clouds)
    ...
    with computation(FORWARD):
        ...
        with interval(1, None):
            dr = (
                dr[0, 0, -1]  # type: ignore[index]
                + delt[0, 0, -1] * (b[0, 0, -1] - 0.5 * dr[0, 0, -1])  # type: ignore[index]
            ) / (1.0 + 0.5 * delt[0, 0, -1])
    ...
    with computation(BACKWARD):
        with interval(-1, None):
            ...
            lw = ur - dr
        with interval(1, -1):
            ur = (ur[0, 0, 1] + delt[0, 0, 0] * (b[0, 0, 0] - 0.5 * ur[0, 0, 1])) / (
                1.0 + 0.5 * delt[0, 0, 0]
            )
            lw = ur - dr
